...
Uses **Jira Cloud REST API v3** via the `mcp-atlassian` MCP server (`sooperset/mcp-atlassian`).
Instance: `redhat.atlassian.net`

> **Note:** Use `jira_search_fields` tool to discover or verify field IDs — they are instance-specific.
...
### 7.2 Custom Fields (redhat.atlassian.net)

Jira projects are either **classic** or **team-managed** — the project type affects which fields and approaches to use. When in doubt, check the project settings or use `jira_search_fields` to confirm available fields.

| Field            | Field ID               | Classic project                                      | Team-managed project             |
| ---------------- | ---------------------- | ---------------------------------------------------- | -------------------------------- |
| Epic label       | `customfield_10011`    | Set on Epic to give it a short display name          | Not needed — use summary         |
| Epic Link        | `customfield_10014`    | Set on Story/Task/Spike to link to an Epic           | Not used — use `parent` instead  |
| Parent (Epic)    | —                      | Not used                                             | `"parent": "PROJ-1"` (plain string, not a dict) |
| Story Points     | `customfield_10028`    | Set via `jira_update_issue` after creation (see 7.3) | Same                             |
| Git Pull Request | `customfield_10875`    | Plain text / URL                                     | Same                             |

### 7.3 Story Points

Story points cannot be set during issue creation. Update after creating:

```
jira_update_issue(
    issue_key="DPROD-123",
    fields={},
    additional_fields={"customfield_10028": 3}
)
```

**Important**: `fields` is a required parameter — pass `{}` when only updating custom fields via `additional_fields`. Both must be dict objects, not JSON strings.
...
Use `jira_create_issue_link` with these verified link type names:
...
### 7.7 Description Format

Pass plain text or markdown — the `mcp-atlassian` tool converts it to Atlassian Document Format (ADF) automatically. Do not hand-craft ADF JSON.
